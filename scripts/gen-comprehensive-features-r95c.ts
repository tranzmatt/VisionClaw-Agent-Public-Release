...
function buildText(): string {
  ...
  t += `Updated: ${DATE} (R95.c — TIER-1 outbound-gate completeness pass)\n`;
  t += "================================================================================\n\n";
  ...
  t += "## CURRENT SECURITY POSTURE (post-R95.c)\n\n";
...
(async () => {
  ...
  const pdfRes = await generateStyledPdf({
    ...
    subtitle: `Comprehensive Features & Platform Reference — ${DATE} (R95.c)`,
    companyLines: [
    ...
    sections: [
      {
        ...
        bullets: [
          ...
          "16 active personas, 275 tools, 62 skills, 150 tables, 425 indexes",
      ...
      {
        title: "Current Security Posture (post-R95.c)",
        bullets: [
  ...
  await sendEmail({
    inboxId,
    to: ownerEmail,
    subject: `VisionClaw Updated Features — PDF + Text (${DATE}, R95.c)`,
    text: `Updated comprehensive features document set for VisionClaw Agent — ${DATE}.

This release captures everything through R95.c (TIER-1 outbound-gate completeness pass):
- 8 outbound surfaces hardened (webhook-relay SSRF + R95 gate, centralized email gate, MCP + glasses response gating, exec lstat+realpath, Felix tenant fail-open removed, recurring-messages tenant scoping, 3 runWithTenant TS errors)
- Plus CI hardening (sql-raw callsite snapshot refactored to content-only comparator)
- Suite 111/111 PASS

Live counts:
- ${STATS.tools} tools, ${STATS.skills} skills, ${STATS.personas} personas
- ${STATS.tables} DB tables, ${STATS.indexes} DB indexes
- ${STATS.tsFiles} TS files, ${STATS.locTotal.toLocaleString()} LOC
...
Public mirror (just refreshed): https://github.com/Huskyauto/VisionClaw-Agent-Public-Release
